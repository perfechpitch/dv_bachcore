...
def main():
    ...
    with open(args.vip_cfg, "r") as cfg_file:
        cfg = json.load(cfg_file)
    axi = cfg.get("axi", {})
    for field, default in (("addr_width", 32), ("id_width", 4), ("len_width", 8)):
        plan["axi_" + field] = axi.get(field, default)
    plan["max_burst_len"] = axi.get("max_burst_len", 16)
    plan["support_narrow_burst"] = cfg.get("support", {}).get("narrow_burst", False)
    # Reject stale 32-bit register strides when targeting a 256-bit profile.
    # The original workbook remains a valid 32-bit demo; new sized traffic can
    # be supplied in an independent JSON plan without rewriting that workbook.
