...
OPTIONAL_TYPES = ("optional_int", "optional_string")
...
def parse_value(text, type_text, label):
    ...
    # An absent cell is unspecified; words such as none/no/null and 0 are data.
    # Check before stripping so whitespace text cannot masquerade as a blank.
...
def load_vip_cfg(ref):
    ...
    cfg = OrderedDict()
    # Older workbooks use one sheet per runtime section. Requirements remain
    # optional so these files still load, while a present metadata sheet is kept.
    sections = VIP_SECTIONS + (("requirements",) if "requirements" in workbook else ())
