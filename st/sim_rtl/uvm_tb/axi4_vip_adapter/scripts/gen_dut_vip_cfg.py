...
import gen_vip_cfg
from vip_workbook import OPTIONAL_TYPES, parse_value, value_text
...
def load_parameter_rows(ref, label, require_value):
    ...
    for row_num, row in enumerate(rows, start=2):
        ...
        # Preserve the difference between a physically blank cell and text.
        ...
        if require_value and clean(value) == "" and type_text.lower() not in OPTIONAL_TYPES + ("null",):
    ...
    with zipfile.ZipFile(path, "r") as workbook_zip:
        ...
        for row_num, values in enumerate(matrix, start=1):
            ...
            for col_index, value in enumerate(values):
                ...
                if cell is None:
                    cell = ET.Element(tag(MAIN_NS, "c"))
                    cell.attrib["r"] = "%s%d" % (column_name(col_index), row_num)
                    insert_at = len(row_element)
                    for child_index, child in enumerate(row_element):
                        child_col = column_index(child.attrib.get("r"))
                        if child_col is not None and child_col > col_index:
                            insert_at = child_index
                            break
                    row_element.insert(insert_at, cell)
                ...
                if rendered == "":
                    # Leave a genuinely empty styled cell, not the text null.
                    continue
