package axi4_vip_adapter_pkg;
  ...
  parameter bit [((AXI_QOS_WIDTH > 0) ? AXI_QOS_WIDTH : 1)-1:0] AXI_DEFAULT_QOS = VIP_AXI_DEFAULT_QOS;
  parameter bit [((AXI_REGION_WIDTH > 0) ? AXI_REGION_WIDTH : 1)-1:0] AXI_DEFAULT_REGION = VIP_AXI_DEFAULT_REGION;
  parameter bit [((AXI_AWUSER_WIDTH > 0) ? AXI_AWUSER_WIDTH : 1)-1:0] AXI_DEFAULT_AWUSER = VIP_AXI_DEFAULT_AWUSER;
  parameter bit [((AXI_ARUSER_WIDTH > 0) ? AXI_ARUSER_WIDTH : 1)-1:0] AXI_DEFAULT_ARUSER = VIP_AXI_DEFAULT_ARUSER;
  parameter bit [((AXI_WUSER_WIDTH > 0) ? AXI_WUSER_WIDTH : 1)-1:0] AXI_DEFAULT_WUSER = VIP_AXI_DEFAULT_WUSER;
  parameter int AXI_MAX_OUTSTANDING_TOTAL = VIP_AXI_MAX_OUTSTANDING_TOTAL;
  // One canonical type for config_db set/get, adapters and future monitors.
  ...
  // Requests use bus-lane data (no implicit shift). beat_count is AxLEN + 1.
  class axi4_burst_request extends uvm_object;
    `uvm_object_utils(axi4_burst_request)
    bit [AXI_ID_WIDTH-1:0] id = AXI_DEFAULT_ID;
    bit [AXI_ADDR_WIDTH-1:0] addr;
    int unsigned beat_count = 1;
    int unsigned size = $clog2(AXI_STRB_WIDTH);
    int unsigned burst = 1;
    bit lock = 0;
    logic [AXI_DATA_WIDTH-1:0] data[];
    logic [AXI_STRB_WIDTH-1:0] strb[];
    // Raw-response callers may opt out of automatic OKAY checking.
    bit check_response = 1;

    function new(string name = "axi4_burst_request");
      super.new(name);
    endfunction

    virtual function void do_copy(uvm_object rhs);
      axi4_burst_request other;
      super.do_copy(rhs);
      if (!$cast(other, rhs)) begin
        `uvm_fatal("AXI_ILLEGAL", "Request copy requires axi4_burst_request")
        return;
      end
      id = other.id; addr = other.addr; beat_count = other.beat_count;
      size = other.size; burst = other.burst; lock = other.lock;
      data = other.data; strb = other.strb; check_response = other.check_response;
    endfunction

    function logic [AXI_STRB_WIDTH-1:0] lane_mask(input int unsigned beat_index);
      logic [AXI_STRB_WIDTH-1:0] mask;
      bit [AXI_ADDR_WIDTH+16:0] beat_addr;
      int unsigned lane, bytes_per_beat;
      mask = '0;
      if (size > $clog2(AXI_STRB_WIDTH)) return mask;
      bytes_per_beat = 1 << size;
      beat_addr = addr;
      beat_addr += beat_index * bytes_per_beat;
      lane = int'(beat_addr % AXI_STRB_WIDTH);
      for (int unsigned b = 0; b < bytes_per_beat; b++) mask[lane+b] = 1'b1;
      return mask;
    endfunction

    // Side-effect-free legality validation is also used by asynchronous schedulers.
    // Safety restrictions are independent of optional checker reporting switches.
    function string validate(input bit is_write);
      int unsigned bytes_per_beat;
      bit [AXI_ADDR_WIDTH+16:0] last_addr;
      if (beat_count == 0 || beat_count > 256 ||
          beat_count > (1 << AXI_LEN_WIDTH) || beat_count > AXI_MAX_BURST_LEN)
        return "beat_count exceeds configured/AXI LEN limits or is zero";
      if (burst != 1 || !axi4_vip_cfg_pkg::VIP_SUPPORT_INCREMENTING_BURST)
        return "Only enabled INCR transactions are implemented";
      if (lock) return "Exclusive/locked transactions are not implemented";
      if (size > $clog2(AXI_STRB_WIDTH)) return "SIZE exceeds bus width";
      bytes_per_beat = 1 << size;
      if ((addr % bytes_per_beat) != 0)
        return "Unaligned transactions are not implemented";
      if (beat_count > 1 && bytes_per_beat < AXI_STRB_WIDTH &&
          !axi4_vip_cfg_pkg::VIP_SUPPORT_NARROW_BURST)
        return "Narrow multi-beat transactions are disabled";
      last_addr = addr;
      last_addr += beat_count * bytes_per_beat - 1;
      if ((last_addr >> AXI_ADDR_WIDTH) != 0) return "Transaction overflows address width";
      if ((addr % 4096) + beat_count * bytes_per_beat > 4096)
        return "INCR transaction crosses a 4KB boundary";
      if (is_write) begin
        if (data.size() != beat_count || strb.size() != beat_count)
          return "Write data/strb array sizes must equal beat_count";
        foreach (strb[i]) begin
          if ($isunknown(strb[i]) || ((strb[i] & ~lane_mask(i)) != '0))
            return "WSTRB contains unknown or out-of-transfer byte lanes";
          if (!axi4_vip_cfg_pkg::VIP_SUPPORT_BYTE_STROBE && strb[i] != lane_mask(i))
            return "Partial WSTRB is disabled";
        end
      end
      return "";
    endfunction
  ...
  class axi4_burst_response extends uvm_object;
    `uvm_object_utils(axi4_burst_response)
    logic [AXI_ID_WIDTH-1:0] bid = 'x;
    logic [1:0] bresp = 'x;
    logic [AXI_ID_WIDTH-1:0] rid[];
    logic [AXI_DATA_WIDTH-1:0] data[];
    logic [1:0] rresp[];
    logic rlast[];
    int unsigned completed_beats = 0;
    function new(string name = "axi4_burst_response");
      super.new(name);
    endfunction
  endclass

  typedef enum int unsigned {
    AXI_COMPLETION_PENDING,
    AXI_COMPLETION_OK,
    AXI_COMPLETION_RESET,
    AXI_COMPLETION_TIMEOUT,
    AXI_COMPLETION_PROTOCOL_ERROR,
    AXI_COMPLETION_REJECTED
  } axi_completion_status_e;

  // A submission owns one persistent completion handle. The adapter publishes
  // response/status/detail before setting done so a late waiter cannot miss it.
  class axi4_completion extends uvm_object;
    `uvm_object_utils(axi4_completion)
    bit done = 0;
    axi_completion_status_e status = AXI_COMPLETION_PENDING;
    axi4_burst_response response;
    string detail;
    longint unsigned serial;
    longint unsigned owner_id;

    function new(string name = "axi4_completion");
      super.new(name);
    endfunction
  endclass
  ...
  class axi4_vip_adapter_base extends uvm_object;
    ...
    // Register the persistent component owner before any sequence can call
    // init(). A shared adapter must have exactly one background service owner.
    virtual function void claim_service_owner();
      if (service_owned) begin
        `uvm_fatal("AXI_SERVICE_OWNER", "Adapter service already has an owner")
        return;
      end
      service_owned = 1;
    endfunction
    ...
    virtual task submit_write(input axi4_burst_request req, output axi4_completion handle);
      handle = null;
      `uvm_fatal("AXI_UNIMPLEMENTED", "submit_write is not implemented")
    endtask

    virtual task submit_read(input axi4_burst_request req, output axi4_completion handle);
      handle = null;
      `uvm_fatal("AXI_UNIMPLEMENTED", "submit_read is not implemented")
    endtask

    virtual task wait_completion(input axi4_completion handle, output axi4_burst_response rsp);
      rsp = null;
      `uvm_fatal("AXI_UNIMPLEMENTED", "wait_completion is not implemented")
    endtask

    virtual function void configure_outstanding(input int unsigned read_limit,
                                                input int unsigned write_limit,
                                                input int unsigned total_limit);
      `uvm_fatal("AXI_UNIMPLEMENTED", "configure_outstanding is not implemented")
    endfunction

    // Actual outstanding counts exclude requests still in software queues.
    virtual function int unsigned get_outstanding_reads();
      return 0;
    endfunction
    virtual function int unsigned get_outstanding_writes();
      return 0;
    endfunction
    virtual function int unsigned get_outstanding_total();
      return 0;
    endfunction
    virtual function int unsigned get_queued_reads();
      return 0;
    endfunction
    virtual function int unsigned get_queued_writes();
      return 0;
    endfunction
    ...
    virtual task axi_write_burst(input axi4_burst_request req, output axi4_burst_response rsp);
      `uvm_fatal("AXI_UNIMPLEMENTED", "axi_write_burst is not implemented")
    endtask
    virtual task axi_read_burst(input axi4_burst_request req, output axi4_burst_response rsp);
      `uvm_fatal("AXI_UNIMPLEMENTED", "axi_read_burst is not implemented")
    endtask

    // Caller owns each channel, invokes init first and preserves AW/W FIFO order.
    // Do not mix these raw primitives with the blocking transaction API.
    virtual task send_aw(input axi4_burst_request req);
      `uvm_fatal("AXI_UNIMPLEMENTED", "send_aw is not implemented")
    endtask
    virtual task send_w(input axi4_burst_request req);
      `uvm_fatal("AXI_UNIMPLEMENTED", "send_w is not implemented")
    endtask
    virtual task send_ar(input axi4_burst_request req);
      `uvm_fatal("AXI_UNIMPLEMENTED", "send_ar is not implemented")
    endtask
    virtual task receive_b(output logic [AXI_ID_WIDTH-1:0] id, output logic [1:0] resp);
      `uvm_fatal("AXI_UNIMPLEMENTED", "receive_b is not implemented")
    endtask
    virtual task receive_r(output logic [AXI_ID_WIDTH-1:0] id,
                           output logic [AXI_DATA_WIDTH-1:0] data,
                           output logic [1:0] resp, output logic last);
      `uvm_fatal("AXI_UNIMPLEMENTED", "receive_r is not implemented")
    endtask
    ...
    virtual task axi_write(
      ...
      if (AXI_ENABLE_RESPONSE_CHECKS && bresp !== AXI_RESP_OKAY) begin
        `uvm_error("AXI_RESPONSE", $sformatf(
  ...
  class axi4_sequence_base extends uvm_sequence #(uvm_sequence_item);
    ...
    `include "axi4_sequence_payload.svh"
    ...
    function automatic bit [AXI_ADDR_WIDTH-1:0] axi_parse_addr_text(
      ...
      if ($sscanf(axi_checked_hex_token(value_text, AXI_ADDR_WIDTH, field_name), "%h", value) != 1) begin
    ...
    function automatic bit [AXI_DATA_WIDTH-1:0] axi_parse_data_text(
      ...
      if ($sscanf(axi_checked_hex_token(value_text, AXI_DATA_WIDTH, field_name), "%h", value) != 1) begin
        `uvm_fatal(get_type_name(), $sformatf("Invalid hex for %s: %s", field_name, value_text))
      end
      return value;
    endfunction
    ...
    function automatic int unsigned axi_parse_uint_text(
      ...
      return axi_checked_uint_text(value_text, field_name);
    endfunction
    ...
    task axi_write_by_mode(
      ...
      axi_write_by_mode_with_gap(addr, data, write_mode, "");
    endtask
    ...
    task axi_write_by_mode_with_gap(
      ...
      axi_check_range(addr, AXI_STRB_WIDTH, 1, write_mode);
      case (write_mode)
        ...
        SINGLE_ADDR_SINGLE_BYTE: begin
          for (int byte_idx = 0; byte_idx < AXI_STRB_WIDTH; byte_idx++) begin
            axi_write_byte_checked(addr + byte_idx, data[8 * byte_idx +: 8]);
            axi_wait_tr_gap_text(tr_gap);
          end
        end
        default: begin
