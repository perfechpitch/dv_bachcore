...
scp_bach_ctrl_m0 改造的独立配置、工作簿保护、回归及部署入口见 [集成说明](integration.md)。需要保持原 vip_cfg.xlsx 不变时，请使用该说明中的 isolated_run / regression 入口。

2026-09-16 三张 VIP sheet 从55项扩展到70项，历史说明见 [工作簿字段说明](workbook_fields_20260916.md)。2026-09-17 用户进一步授权按master设置DUT并生成FINAL：工作簿当前FINAL为DATA256/ADDR32/ID8/LEN4、INCR、读/写/总上限128/128/128；最大burst继承BASE=1，见 [09配置记录](workbook_master_activation_20260917.md)。BASE及其锁保持不变。生产active JSON、`tb/generated`及用户sequence输入继续保留原配置，仍可运行原DATA32示例；工作簿更新不自动覆盖这些文件。1000MHz和24GB/s等requirements记录仍是元数据。
...
| `FINAL_FEATURE` | 保存合并后的配置及需求记录，并在 `source` 中标记来源 | 不修改，由 `make vip_cfg` 生成 |
...
2. 在 `BASE_VIP_CFG.value` 中给出默认值；`optional_int/optional_string` 允许真实空白表示未指定，`DUT_BUFF_FEATURE.value` 默认留空继承。
3. 在 `scripts/gen_vip_cfg.py` 和对应 AXI adapter 中接入运行参数。`requirements` 仅保存需求数据，在生成器中验证，不能视为已实现功能。
...
axi4_sequence_payload.svh
checker/axi4_checker_pkg.sv
checker/axi4_protocol_monitor.sv
checker/axi4_configured_monitor.svh
...
`axi.data_width` 必须是8到1024之间的2次幂，WSTRB位宽为DATA/8。修改位宽后，还需要检查 seq 中的地址、数据、`write_mode` 和目标 DUT 接口是否匹配新位宽。
