interface ts_if(input bit clk,input bit reset_n);
  ...
  logic [7:0] ts2dtecore_task_pid,ts2mucore_task_pid,ts2vucore_task_pid;
  logic [1:0] ts2dtecore_task_vcid,ts2mucore_task_vcid,ts2vucore_task_vcid;
  ...
  logic [7:0] dtecore2ts_done_pid,mucore2ts_done_pid,vucore2ts_done_pid;
  ...
  clocking drv_cb @(posedge clk);
    ...
    output ts2dtecore_task_pid,ts2mucore_task_pid,ts2vucore_task_pid;
    output ts2dtecore_task_vcid,ts2mucore_task_vcid,ts2vucore_task_vcid;
  ...
  clocking mon_cb @(posedge clk);
    ...
    input ts2dtecore_task_pid,ts2mucore_task_pid,ts2vucore_task_pid;
    input ts2dtecore_task_vcid,ts2mucore_task_vcid,ts2vucore_task_vcid;
  ...
  property p_vu_stable; @(posedge clk) disable iff(!reset_n)
    ts2vucore_task_valid&&!vucore2ts_task_ready |=> ts2vucore_task_valid&&$stable({ts2vucore_task_uid,ts2vucore_task_tid,ts2vucore_task_streamid,ts2vucore_task_pid,ts2vucore_task_vcid,ts2vucore_task_pc}); endproperty
  a_vu_stable: assert property(p_vu_stable);
  property p_mu_stable; @(posedge clk) disable iff(!reset_n)
    ts2mucore_task_valid&&!mucore2ts_task_ready |=> ts2mucore_task_valid&&$stable({ts2mucore_task_uid,ts2mucore_task_tid,ts2mucore_task_streamid,ts2mucore_task_pid,ts2mucore_task_vcid,ts2mucore_task_pc}); endproperty
