  ...
  task main_phase(uvm_phase phase);
    ...
    forever begin @(vif.mon_cb); if(vif.mon_cb.reset_n) begin
      ...
      if(vif.mon_cb.ts2mucore_task_valid&&vif.mon_cb.mucore2ts_task_ready) begin
        ...
        t.stream_id=vif.mon_cb.ts2mucore_task_streamid; t.pc=vif.mon_cb.ts2mucore_task_pc;
        t.pid=vif.mon_cb.ts2mucore_task_pid; t.vcid=vif.mon_cb.ts2mucore_task_vcid;
        ap.write(t); $fwrite(log_fd,"MU %04h %02h %x %08h %02h %x\n",t.uid,t.tid,t.stream_id,t.pc,t.pid,t.vcid);
      end
      if(vif.mon_cb.ts2vucore_task_valid&&vif.mon_cb.vucore2ts_task_ready) begin
        ...
        t.stream_id=vif.mon_cb.ts2vucore_task_streamid; t.pc=vif.mon_cb.ts2vucore_task_pc;
        t.pid=vif.mon_cb.ts2vucore_task_pid; t.vcid=vif.mon_cb.ts2vucore_task_vcid;
        ap.write(t); $fwrite(log_fd,"VU %04h %02h %x %08h %02h %x\n",t.uid,t.tid,t.stream_id,t.pc,t.pid,t.vcid);
      end
    end end
  endtask
endclass
