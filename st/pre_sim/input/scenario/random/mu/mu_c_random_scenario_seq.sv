class mu_c_random_scenario_seq extends scenario_base_seq;
    `uvm_object_utils(mu_c_random_scenario_seq)

    function new(string name = "mu_c_random_scenario_seq");
        super.new(name);
    endfunction
    ...
    virtual function void configure_tasks();
        ...
        for(int i = 0; i < task_num; i++) begin
            task_id = add_random_task(.rv_core(HART_MU),
                                      .use_start_pc(i == 0),
                                      .start_pc('0));
            set_task_seq_weight(task_id, C_INST_SEQ,      WEIGHT_HIGH);
            ...
            // C_INST_SEQ currently has no configurable sub-sequence selector.
        end
    endfunction
endclass

`RANDOM_SCENARIO_REGISTER(mu_c_random_scenario_seq, "mu_c_random")
