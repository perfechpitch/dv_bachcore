// ============================================================================
// Filename             : mu_reference.sv
// Author               : kippy
// Created On           : 2026-9-18 10:21
// Last Modified        :
// Update Count         :
// Description          :
// ============================================================================

`ifndef MU_REFERENCE_SV
`define MU_REFERENCE_SV

//`uvm_analysis_imp_decl(_mu)

class mu_reference extends uvm_component;

    //uvm_analysis_imp_mu  #(mu_seq_item,mu_reference) mu_imp;
    //uvm_analysis_port              #(mu_seq_item)                        mu_ap;

    // 
    // This property is the virtual interfaced needed for this component to drive
    // and view HDL signals.
    //
    //mu_config               mu_cfg;
    // add for mul_core evironment,this environment will be  may times  which are equal to core_num
    // xxx_cfg is env top config,eg if it is scache,xxx_cfg is scache_cfg
    //xxx_config                          xxx_cfg;

    // Opitional variable.
    //
    // Declare variable as file handle.
    //
    // For example:
    // int      mu_ref_log;
   
    // Transfer package
    //protected mu_seq_item  mu_q[$];
    //protected mu_seq_item  mu_tr;
    
    //function covergroup :(eg:outstanding/out-of-order)
    covergroup cov_trans;
        option.per_instance = 1;
    //    trans_start_func : coverpoint ref_tr.func {
    //        option.auto_bin_max = 16; 
    //    }
    //    trans_func0  : coverpoint ref_tr.read_write;
    //    trans_func1 : coverpoint ref_tr.func1 {
    //        bins func1s[] = {1, 2, 4, 8};
    //        illegal_bins invalid_func1s = default; 
    //    }
    //    trans_func0Xfunc1 : cross trans_func0, trans_func1;
    endgroup : cov_trans

    `uvm_component_utils_begin(mu_reference)
    `uvm_component_utils_end
    
    // new - constructor
    function new (string name, uvm_component parent);
        super.new(name, parent);
        cov_trans = new();
        cov_trans.set_inst_name({get_full_name(), ".cov_trans"});

        //
        // Instance analysis port
        //
        //mu_imp = new("mu_imp", this);
        //mu_ap = new("mu_ap", this);
    endfunction : new
    
    function void build_phase(uvm_phase phase);
        super.build_phase(phase);
        //
        // Retrieve the configuration and the virtual interface.
        //
        //if(!uvm_config_db#(mu_config)::get(this,"","mu_cfg",mu_cfg))
        //    `uvm_fatal("NOCFG",{"config must be set for: ",get_full_name(),".mu_cfg"});

        // add for mul_core evironment,this environment will be  may times  which are equal to core_num
        // xxx_cfg is env top config,eg if it is scache,xxx_cfg is scache_cfg
        // if(!uvm_config_db#(xxx_config)::get(this,"","xxx_cfg",xxx_cfg))
        //     `uvm_fatal("NOCFG",{"config must be set for: ",get_full_name(),".xxx_cfg"});

        // Specify uvm_info tag "MU_REF" with an action ,
        // which argument can take the value UVM_LOG. The uvm_info with 
        // tag "MU_REF" will print messages to a file pointed by  
        // the filehandle mu_ref_log.
        // 
        // two choice for log_name
        // 1: for one_core evironment
        // mu_ref_log = $fopen("./log/mu.ref.log","w");
        //
        // 2: for mul_core evironment,this environment will be  may times  which are equal to core_num
        //    to avoild log confusedly in mul_core environment,we should split logs to one log per core
        // mu_ref_log = $fopen($sformatf("./log/core%0d_mu.ref.log",xxx_cfg.core_num),"w");
        //
        // set_report_id_action("MU_REF",UVM_LOG);
        // set_report_id_file("MU_REF",mu_ref_log);
    endfunction: build_phase
   
    //function write_mu(mu_seq_item tr);
          //
          //ref exe
          //
          //cov_trans.sample();
    //endfunction :write_mu

    extern virtual task     reset_phase(uvm_phase phase);
    extern virtual function ref_reset();
endclass : mu_reference

task mu_reference::reset_phase(uvm_phase phase);
    ref_reset();
endtask : reset_phase

function mu_reference::ref_reset();
    // User should reset all queues and internal variables.
    //
    // ToDo
    //mu_q.delete();
endfunction :ref_reset 
`endif